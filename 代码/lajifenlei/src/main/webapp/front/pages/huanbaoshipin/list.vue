<template>
	...
	<view class="cu-bar bg-white search" :style="[{top:CustomBar + 'px'}]">
        <view class="search-form round">
				<input v-model="searchForm.huanbaoshipinName" type="text" placeholder="视频标题"></input>
		</view>
	...
	<scroll-view scroll-x="true" class="tabView"
		...
		<view :style='categoryName===item.codeIndex?sactiveItem:sitem' class="tab"
			  v-for="(item,index) in huanbaoshipinTypesList" :key="index"
...
<view class="list" :style='{"borderRadius":0,"backgroundColor":"rgba(255, 255, 255, 0)"}'>
    <view style="margin: 10rpx 10rpx 10rpx 10rpx;"
        ...
        <image :style='{"borderRadius":"40rpx","width":"50%","height":"200rpx"}' mode="aspectFill" class="listmpic"
               :src="product.huanbaoshipinPhoto?product.huanbaoshipinPhoto.split(',')[0]:''"></image>
        <view class="listmr">
           ...
           <view style="display: flex;justify-content: space-between;float: left;margin-top: 20%;">
            <text style="margin-left: 50rpx;" v-if="isAuth('huanbaoshipin','修改')" class="cuIcon-edit" @click.stop="onUpdateTap(product.id)">
                修改
            </text>
            <text v-if="isAuth('huanbaoshipin','删除')" class="cuIcon-delete"
            ...
            v-if="isAuth('huanbaoshipin','新增')" class="add-btn" @click="onAddTap()">新增</button>
	...
	export default {
		...
		async onShow() {
            ...
            _this.btnColor = _this.btnColor.sort(() => {
		  	...
		  	let huanbaoshipinTypes = await this.$api.page('dictionary', {
			 	 page: 1,
			  	limit: 100,
		  		dicCode:'huanbaoshipin_types'
		  	});
		  	this.huanbaoshipinTypesList = huanbaoshipinTypes.data.list;
		...
		methods: {
			//查询条件切换
			queryChange(e) {
				this.searchForm.huanbaoshipinName = "";
			},
			//类别搜索
				...
				let params = {
					...
					limit: mescroll.size
							...
							if (this.categoryName != 0) {
                                params.huanbaoshipinTypes = this.categoryName
                            }

                let res = await this.$api.list(`huanbaoshipin`, params);
			...
			onDeleteTap(id) {
				...
				uni.showModal({
					...
					success: async function(res) {
						if (res.confirm) {
							await _this.$api.del('huanbaoshipin', JSON.stringify([id]));
							_this.hasNext = true
							// 重置分页参数页数为1
			...
			async search() {
				...
				if (this.searchForm.huanbaoshipinName) {
                    searchForm['huanbaoshipinName'] = this.searchForm.huanbaoshipinName
                }
				if (this.searchForm.huanbaoshipinName) {
					searchForm['huanbaoshipinName'] = this.searchForm.huanbaoshipinName
				}

                let res = await this.$api.list(`huanbaoshipin`, searchForm);
				// 如果是第一页数据置空
