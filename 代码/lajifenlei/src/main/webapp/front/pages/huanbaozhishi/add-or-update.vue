        ...
        <form class="app-update-pv">
            <view
                ...
                <input   disabled
                         ...
                         :disabled="ro.huanbaozhishiName" type="text" v-model="ruleForm.huanbaozhishiName" placeholder="标题"></input>
            </view>
                <view
                        ...
                        class="cu-form-group" @tap="huanbaozhishiPhotoTap">
                    <view :style='{"width":"160rpx","fontSize":"28rpx","color":"#6a4826","textAlign":"left"}'
                          class="title">封面</view>
                    <view class="right-input" style="padding:0;textAlign:left">
                        <image
                                ...
                                class="avator" v-if="ruleForm.huanbaozhishiPhoto" :src="ruleForm.huanbaozhishiPhoto" mode="aspectFill">
                        </image>
            ...
            <view
                ...
                <textarea :style='{"boxShadow":"0 0 0px rgba(0,0,0,.6) inset","backgroundColor":"rgba(255, 255, 255, 1)","borderColor":"#6a4826","borderRadius":"16rpx","color":"#6a4826","textAlign":"left","borderWidth":"2rpx","fontSize":"28rpx","borderStyle":"solid","height":"88rpx","marginTop":"7rpx","height":"200rpx"}'
                          :disabled="ro.huanbaozhishiContent" v-model="ruleForm.huanbaozhishiContent" placeholder="内容"/>
            </view>
    ...
    export default {
        ...
        async onLoad(options) {
            ...
            if (options.id) {
                ...
                // 获取信息
                let res = await this.$api.info(`huanbaozhishi`, this.ruleForm.id);
                this.ruleForm = res.data;
            }
            if(options.huanbaozhishiId){
                this.ruleForm.huanbaozhishiId = options.huanbaozhishiId;
            }
            // 跨表
            // this.styleChange()
        },
        methods: {
			...
			huanbaozhishiPhotoTap() {
                let _this = this;
                this.$api.upload(function(res) {
                    _this.ruleForm.huanbaozhishiPhoto = _this.$base.url + 'upload/' + res.file;
                    _this.$forceUpdate();
                    _this.$nextTick(() => {
                        // _this.styleChange()
                    })
            ...
            async onSubmitTap() {
				if ((!this.ruleForm.huanbaozhishiName)) {
                    this.$utils.msg(`标题不能为空`);
                    return
                }
				if ((!this.ruleForm.huanbaozhishiPhoto)) {
                    this.$utils.msg(`封面不能为空`);
                    return
                }
                if (this.ruleForm.id) {
                    await this.$api.update(`huanbaozhishi`, this.ruleForm);
                } else {
                    await this.$api.save(`huanbaozhishi`, this.ruleForm);
                }
