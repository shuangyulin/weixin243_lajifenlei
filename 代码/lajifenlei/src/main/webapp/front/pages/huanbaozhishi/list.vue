<template>
	...
	<view class="cu-bar bg-white search" :style="[{top:CustomBar + 'px'}]">
        <view class="search-form round">
				<input v-model="searchForm.huanbaozhishiName" type="text" placeholder="标题"></input>
		</view>
...
<view class="list" :style='{"borderRadius":0,"backgroundColor":"rgba(255, 255, 255, 0)"}'>
    <view style="margin: 10rpx 10rpx 10rpx 10rpx;"
        ...
        <image :style='{"borderRadius":"40rpx","width":"50%","height":"200rpx"}' mode="aspectFill" class="listmpic"
               :src="product.huanbaozhishiPhoto?product.huanbaozhishiPhoto.split(',')[0]:''"></image>
        <view class="listmr">
            <view class="col3 f30 elip mb15"
                  :style='{"fontSize":"28rpx","lineHeight":"56rpx","color":"rgba(255, 255, 255, 1)","textAlign":"center"}'>
                {{product.huanbaozhishiName}}
            </view>
        </view>
           <view style="display: flex;justify-content: space-between;float: left;margin-top: 20%;">
            <text style="margin-left: 50rpx;" v-if="isAuth('huanbaozhishi','修改')" class="cuIcon-edit" @click.stop="onUpdateTap(product.id)">
                修改
            </text>
            <text v-if="isAuth('huanbaozhishi','删除')" class="cuIcon-delete"
            ...
            v-if="isAuth('huanbaozhishi','新增')" class="add-btn" @click="onAddTap()">新增</button>
...
<script>
	export default {
		...
		methods: {
			//查询条件切换
			queryChange(e) {
				this.searchForm.huanbaozhishiName = "";
			},
			//类别搜索
			...
			async upCallback(mescroll) {
                ...
                let res = await this.$api.list(`huanbaozhishi`, params);
			...
			onDeleteTap(id) {
				...
				uni.showModal({
					...
					success: async function(res) {
						if (res.confirm) {
							await _this.$api.del('huanbaozhishi', JSON.stringify([id]));
							_this.hasNext = true
							// 重置分页参数页数为1
			...
			async search() {
				...
				if (this.searchForm.huanbaozhishiName) {
                    searchForm['huanbaozhishiName'] = this.searchForm.huanbaozhishiName
                }
				if (this.searchForm.huanbaozhishiName) {
					searchForm['huanbaozhishiName'] = this.searchForm.huanbaozhishiName
				}

                let res = await this.$api.list(`huanbaozhishi`, searchForm);
				// 如果是第一页数据置空
